...
Project.findById = ({ platformId, projectId }, result) => {
  ...
  const mailSql = sqlstring.format(
    `
      SELECT
        fundings.*,
        funding_timeline.timeline
      FROM fundings 
      LEFT JOIN funding_timeline 
        ON funding_timeline.source = fundings.source
        AND funding_timeline.source_id = fundings.source_id
      WHERE
        fundings.source = ?
        AND fundings.source_slug = ?
      ORDER BY fundings.id DESC
      LIMIT 1;
    `,[
      platformId,
      projectId,
